...
pagina_mp3_to_wav::pagina_mp3_to_wav(wstring director)
{
    ...
    if (!drept_ex_1.loadFromFile(project_director+"/assets/Media/mp3_to_wav/dreptunghi_exit.png")) {
    ...
    if (!drept_ex_2.loadFromFile(project_director+"/assets/Media/mp3_to_wav/dreptunghi_exit_1.png")) {
    ...
    if (!sag_ex.loadFromFile(project_director+"/assets/Media/mp3_to_wav/sageata_exit.png")) {
    ...
    if (!drept_1.loadFromFile(project_director+"/assets/Media/mp3_to_wav/dreptunghi.png")) {
    ...
    if (!sag.loadFromFile(project_director+"/assets/Media/mp3_to_wav/sageata.png")) {
    ...
    if (!poz_rotatie[0].loadFromFile(project_director+"/assets/Media/mp3_to_wav/wav_0.png")) {
        mesaj_eroare="Eroare la incarcare logo wav_0!\n";
        eroare=true;
    }


    //incarcare logo WAV 1
    if (!poz_rotatie[1].loadFromFile(project_director+"/assets/Media/mp3_to_wav/wav_1.png")) {
        mesaj_eroare="Eroare la incarcare logo wav_1!\n";
        eroare=true;
    }


    //incarcare logo WAV 2
    if (!poz_rotatie[2].loadFromFile(project_director+"/assets/Media/mp3_to_wav/wav_2.png")) {
        mesaj_eroare="Eroare la incarcare logo wav_2!\n";
        eroare=true;
    }


    //incarcare logo WAV 3
    if (!poz_rotatie[3].loadFromFile(project_director+"/assets/Media/mp3_to_wav/wav_3.png")) {
        mesaj_eroare="Eroare la incarcare logo wav_3!\n";
        eroare=true;
    }


    //incarcare logo WAV 4
    if (!poz_rotatie[4].loadFromFile(project_director+"/assets/Media/mp3_to_wav/wav_4.png")) {
        mesaj_eroare="Eroare la incarcare logo wav_4!\n";
        eroare=true;
    }


    //incarcare logo WAV 5
    if (!poz_rotatie[5].loadFromFile(project_director+"/assets/Media/mp3_to_wav/wav_5.png")) {
        mesaj_eroare="Eroare la incarcare logo wav_5!\n";
        eroare=true;
    }


    //incarcare logo WAV 6
    if (!poz_rotatie[6].loadFromFile(project_director+"/assets/Media/mp3_to_wav/wav_6.png")) {
        mesaj_eroare="Eroare la incarcare logo wav_6!\n";
        eroare=true;
    }


    //incarcare logo WAV 7
    if (!poz_rotatie[7].loadFromFile(project_director+"/assets/Media/mp3_to_wav/wav_7.png")) {
        mesaj_eroare="Eroare la incarcare logo wav_7!\n";
        eroare=true;
    }


    //incarcare logo WAV 8
    if (!poz_rotatie[8].loadFromFile(project_director+"/assets/Media/mp3_to_wav/wav_8.png")) {
        mesaj_eroare="Eroare la incarcare logo wav_8!\n";
        eroare=true;
    }


    //incarcare dreptunghi convert albastru
    if (!sub_1.loadFromFile(project_director+"/assets/Media/mp3_to_wav/submit_1.png")) {
    ...
    if (!sub_2.loadFromFile(project_director+"/assets/Media/mp3_to_wav/submit_2.png")) {
    ...
    if (!ch_1.loadFromFile(project_director+"/assets/Media/mp3_to_wav/choose.png")) {
    ...
    if (!ch_2.loadFromFile(project_director+"/assets/Media/mp3_to_wav/choose_1.png")) {
    ...
    //incarcare fundal
    if (!background.loadFromFile(project_director+"/assets/Media/mp3_to_wav/background.png")) {
    ...
    if (!error_box_1.loadFromFile(project_director+"/assets/Media/mp3_to_wav/error_box_1.png")) {
    ...
    if (!error_box_2.loadFromFile(project_director+"/assets/Media/mp3_to_wav/error_box.png")) {
...
Sprite pagina_mp3_to_wav:: desenare(float width, float height, sf::Vector2i mousePos,const tgui::TextArea::Ptr& editBox1,const tgui::TextArea::Ptr& editBox2,Event event) {
...
//configurarea text box urilor
    ...
    editBox1->setDefaultText("Insert the \".mp3\" file here...");
    ...
    draw_figura(renderTexture,text,drept_1,0,
                252,27,"     Convert\n"
                       "  Mp3 to Wav",83,252+180-36-9,27+36);

    //mesaj pentru text boxuri
     draw_text(renderTexture,text,"   Please enter the \".mp3\" file in the box below\n"
                   "   (Introdu fisierul \".mp3\" in caseta de mai jos)",29,252+72+18,225+54);


     draw_text(renderTexture,text,"Please enter the \".wav\" file destination in the box below\n"
                   " (Introdu destinatia fisierului \".wav\" in caseta de mai jos)",29,252+45,504);
        ...
        if(event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left)
        {
            ...
            if (!operatie_in_progres) {
                ...
                std::thread convertThread([&]() {
                    ...
                    convert_x_to_y(id_text_box_1, id_text_box_2, editBox1, editBox2,L".mp3",L".wav");
...
//buton choose de la alegerea fisierului/directorului
    ...
    else
    {
        ...
        if(event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left)
        {
            ...
            locatie_fisier= selecteazaFisier(".mp3");
    ...
    if(operatie_in_progres==true||(poz_sag>7&&poz_sag+198<=507)||rotire==true)
    {
        draw_figura(renderTexture,text,sag,0,
                    1120, (float)180-18+poz_sag,"",0,0,0);
    ...
    //rotirea imaginii de WAV in timpul convertirii (dreapta jos)
    if(rotire==true) {
        id_rotatie++;
    ...
    else if(id_locatie_invalida==true)
    {
        draw_figura(renderTexture, text, error_box_2, 0,
                    36, 499, "Error box:", 35, 63, 510+72+18,sf::Color::Black);
    ...
    else if(id_insertie_invalida==true)
    {
        draw_figura(renderTexture, text, error_box_2, 0,
                    36, 499, "Error box:", 35, 63, 510+72+18,sf::Color::Black);
         draw_text(renderTexture,text,"Invalid\n"
    ...
    else if(id_operatie_reusita==true)
    {
        draw_figura(renderTexture, text, error_box_2, 0,
                    36, 499, "Error box:", 35, 63, 510+72+18,sf::Color::Black);
         draw_text(renderTexture,text,"   Convert\n"
...
void pagina_mp3_to_wav:: resetare_valori(const tgui::TextArea::Ptr& editBox1,const tgui::TextArea::Ptr& editBox2)
{
    id_insertie_invalida=false;
